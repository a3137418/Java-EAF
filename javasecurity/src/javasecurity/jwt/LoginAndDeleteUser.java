...
public class LoginAndDeleteUser {
	public static void main(String[] args) throws Exception {
		// 1.產生 JWT 簽名密鑰
		String signSecret = KeyUtil.generateSecret(32);
		
		// 2.模擬登入(成功之後會得到 token)
		String token =  login("admin", "1234", signSecret);
		if(token == null) {
			System.out.println("燈入失敗");
			return;
		}
		...
		// 3.模擬呼叫刪除會員API
		deleteUserApi(token, signSecret);
		
		System.out.println("\n============================");
		// 4. 模擬呼叫新增會員API
		addUserApi(token, signSecret);
	...
	public static void deleteUserApi(String token , String signingSecret) throws Exception{
		System.out.println("呼叫API: 刪除會員");
		System.out.println("需要權限: DELETE_USER");
...
//				.claim("permission", "USER_DELETE")
		...
		return KeyUtil.signJWT(claimSet, signingSecret);
